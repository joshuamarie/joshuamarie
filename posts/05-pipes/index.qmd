...
A **pipe** operator is a binary operator, just like `+`, that passes the output of one function as the input to the another expression. In R, it mimics Python's chaining method:
...
::: callout-note
In Python, you can't break the code into another line, unless `\` between the methods, or closing the expression with `()` is applied. As you can see, R is more forgiving. 
:::
...
# The Early Days: Pre-Pipe Workflows (≤2012)
...
1. Using right assignment `->` to assign to `.`
...
### `{zeallot}` Multiple Assignment (2018)
