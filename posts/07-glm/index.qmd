...
First, in Linear regression, we have two ways to estimate the coefficients: MLE and ordinary least squares (OLS), which is also derived and closed form from the maximum likelihood of the normal distribution. In GLM, we cannot use OLS, thus the MLE is the only main solution. 

Then for the extra part, which we talk about the deviance. Imagine this as the residual sum of squares in GLMs. 

### Maximum Likelihood Estimation

But how do we actually estimate $\beta$? We use *maximum likelihood estimation* (MLE), but considering there are many forms of likelihood functions, derived from several probability distributions came from exponential family, and so there's several link functions to be used, we have to <continue this>
...
Fancy, isn't it? SIKE!! You don't want this, trust me. This thing is so hard to calculate, even the computer starts to act crazy if you really direcly executed a program for this. Direct maximization of this product is computationally difficult. Taking logarithms simplifies things enormously (thanks, John Napier!). 
...
Since, there's no actual closed form to calculate the coefficients, we can't solve $U(\beta) = 0$ directly. There's a method we can use called **Iteratively Reweighted Least Squares (IRLS)**, which is just Newton-Raphson but with the Fisher Information Matrix.
...
### Be deviant: Deviance (GLM's Version of Sum of Squares) <continue this part>

I am not planning to include this, but I was thinking if anyone is curious on how the fitness in GLM is being measured. In Linear regression, we measure model fit using Residual Sum of Squares (RSS). It is so odd for GLM to use other way to measure (well, the key term is "Generalized", after all) — Instead, GLMs use **deviance**:
...
## Full Function implementation program

The main programming languages are still R and Python. The functions I'll implement below expects to be brittle and not applicable for some cases. 
...
2. The "I swear I'm not crazy" way (calling our own IRLS function in this [part](#irls))
...
### Tier 2: The “I read the math section” way
